...
- 五个可浏览模块：管理概览（含话术优化待办 list/start/close）、话术库、SOP（写操作未接入）、内容管理、系统同步。VOC / 工单 / 离线抽样 / 架构图已从一期导航删除。
- 主内容只支持鼠标滚轮、触控板 / 触控滚动和原生滚动条；鼠标按住拖动不得滚页。最小窗口下不得遮挡关键操作或产生页面级横向滚动。
- 管理概览按统计范围、连续 KPI 条和待办决策表组织。没有冻结字段的影响 / 处理窗口写「未接入」。禁止个人排名与伪经营 KPI。复制不能推断发送、采纳、未修改或正确。
...
- 没有冻结合同的 KPI 写「未接入」，不用 `Date.now` 发明数字。话术库读当前发布 hydrate（hydrate 为空才回退本机检索索引）。
- adopted 只等于复制成功。Owner 发布走 dual-review 再 publish；没有会话时禁用。
...
14. Dashboard：IPC 仍 query-only，原生菜单 / Tray / macOS 程序坞可直接调用 Main；单例、关闭重建、最小化恢复、无 customerAgent、五个模块可浏览（管理概览、话术库、SOP、内容管理、系统同步）；滚轮 / 原生滚动条与最小窗口响应式可用，鼠标拖动不滚页；侧栏可折叠且以稳定 `data-nav-phase` 收口，同一 toggle 不放在 Logo 上，193/192 折叠阈值、208 滞回展开阈值与 lastExpandedWidth 恢复成立，折叠态可拖宽预览且阈值前平滑弹回（含 lostcapture 即使 buttons===1、回滚后 aria-valuenow=平台折叠宽度），跨阈值必须先提交当前 raw 帧再进入 expanding，titlebar/sidebar/main/divider 由唯一 renderedNavWidth 驱动且误差 ≤1（mac 折叠稳态 120，toggle.right≤112；native 折叠 72）；没有产品会话时写「未接入」，Owner 发布走 dual-review。macOS 为 hiddenInset 一体 chrome，原生标题文字不可见，交通灯与 drag / no-drag 安全；Windows / Linux 保持原生窗控；选中导航无左侧紫色竖线，hover 灰、selected 淡紫；主题为单 trigger / menu；980 / 1180 / 1440 无页面级横溢出；main light/dark alpha 1 且无 blur。
