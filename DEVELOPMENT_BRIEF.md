...
- 仅允许独立 `dashboard.ts` preload（话术库只读、内容会话/导入/发布、话术优化待办 list/start/close）；不得进入 overlay `trustedContents()`；不得挂 overlay 的 search / login / copy；renderer 无 Node / Electron / clipboard。
