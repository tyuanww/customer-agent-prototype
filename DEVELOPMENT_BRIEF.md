...
- 十个可浏览模块：管理概览、VOC / 工单洞察、检索效果、离线三维抽样复核、四域话术库、话术优化待办（产品会话下 coach/owner 活列表/开始/关闭，空列表合法；从话术不准自动开单仍要 persist）、SOP 只读过敏树（与坐席窗同一份合成树，不持久化）、内容与发布、公告与同步、架构能力图。
...
- VOC / 工单 / KPI 全部来自 TypeScript 编译期静态 manifest，深冻结；不用 `Date.now` 生成指标；不持久化。话术库读当前发布 hydrate（hydrate 为空才回退本机检索索引），不把 VOC 数字写成生产数据。
