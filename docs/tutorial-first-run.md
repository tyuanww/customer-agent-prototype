...
> **本页默认路径是合成离线浮窗**：合成数据、不连产品 API、不代发。
> 「无后端」只描述这条默认路径和 Dashboard 演示环境，**不表示**产品远端（`product-remote` / 隧道）没有 API。
...
成功时桌面上只剩一颗透明狐狸头。点它会展开玻璃查询胶囊；输入合成问法后出现最多 3 条**原文**话术卡，卡片上没有匹配分。点「复制话术」后按钮变成「已复制」，随后收起回狐狸头。查询胶囊上的 Dashboard 图标会打开第三个标准系统窗；没有产品会话时工作台写「未接入」，不再堆「演示数据」徽标。

本页默认是合成离线浮窗。若走产品远端（`product-remote` / 隧道），出现飞书 / 账号登录窗是预期；清单见 [P4 how-to](how-to-p4-remote-mac.md)。如果出现「已发送」或真实客户原文，说明你不在本 Demo 的预期路径上，请立刻停下来对照本页与 `README.md`。
...
2. 成功标志：玻璃查询胶囊出现（约 600×88）；输入框自动聚焦；未登录占位「登录后查询话术」，有「登录」按钮；右侧「智能检索」是小开关且默认开。胶囊上**没有** `DEMO` / `MOCK AUTH` 徽标。话术卡片不再贴 `DEMO · 合成数据`。
3. 不要把「只复制不代发」当成胶囊必显文案；复制成功只表示进剪贴板。
...
过敏售后切片 1（**合成、可选**，不是发货查询必经步）：输入 `过敏了怎么办`（或 `用了露芷面膜过敏了怎么办`）。成功标志：Top 3 结果标题和卡片之间出现「过敏可走售后流程 · 建议先要凭证」和按钮「打开过敏售后流程」。点按钮后出现独立可拖动的 SOP 窗，首屏是要照片 / 批次的凭证话术。Esc / × 收起后，同一结果应改成「继续过敏售后流程」。Query 复制仍约 900ms 收起查询，不会关掉 SOP；SOP 复制只反馈 1.2s，不进入 Query `COPIED`。`怎么用` + `过敏`、空结果、少发 / 长痘、以及「什么时候发货」都不应出现 SOP 按钮。SOP 卡片没有「话术不准」，也没有 Query 排名「1」。选严重程度时有一句可见提示；凭证步才有「不建单、不指令仓配」。本机已确认该问句可召回。

Query Top 3 每张卡有发丝「话术不准」。点后该卡显示「已记录，待话术师核实」，复制仍是「已复制」，不得出现已处理 / 已发送 / 已上报。同一浮窗再搜同一条仍显示已记录。收起查询后本地记录丢弃。
...
重新点狐狸头打开查询，再点胶囊右侧的线框 Dashboard 图标（`aria-label` 为「打开运营工作台」）。已登录则直接开工作台。已接 loopback 合成栈且未登录时，这个图标会先打开「登录」窗，成功后才开工作台；关掉登录窗则不打开。登录窗分段只有 **飞书 / 账号**：飞书会打开系统浏览器，不在本窗加载授权页；账号表单仍在本窗。S0 日常路径没有 loopback origin，点登录不可用。本机真飞书要仓外 `feishu.env` 与 how-to，不要把 S0 当成真实飞书。狐狸 / 查询右键、「打开运营工作台」、系统 Tray / 应用菜单、macOS 程序坞仍直接打开 / 恢复 Dashboard，不经过胶囊登录门。程序坞点击不会展开查询。

| 可观察成功标志 | 明确不会发生 |
| --- | --- |
| 出现运营工作台标准窗，约 1180×760，可缩放、非置顶、出现在任务栏 | 浮窗自己变成 520×760 工作台 |
| 标题为「客服运营工作台」；没有「演示数据 / 无后端 · 不保存」徽标墙 | 工作台内出现真实客户工单号或发明的 KPI |
| 「话术库」在产品远端列出当前发布 hydrate（空才回退本机索引）；S0 离线才是本机索引 | 话术库单条更新/删除成功（没有冻结合同） |
...
「内容管理」可选择本地 CSV 或 xlsx，解析成待审核草稿。xlsx 只读第一张表。中文表头会映射到场景与标准话术，空白或不完整行会跳过。上限 10MiB / 5000 行。Owner 点发布后：导入 parked 时走 dual-review，再 publish；状态大约每 1.5 秒问一次，429 再等一轮。一期发布仅管理员；话术师可导入产品或活动草稿。售后 / SOP 写库没有冻结合同，按钮保持未接入。

管理概览里的待办在有产品会话且角色为 coach / owner 时，走冻结 `GET /v1/metrics/iteration-tasks` 与 `POST .../start|close`。空列表合法。没有产品会话时写「未接入」，不会回落到 DEMO 5 条。从 Query「话术不准」自动开单仍要 persist。

「SOP」在话术库下面。没有写库合同时，上传 / 更新 / 删除 / 导出 / 自定义步骤点了显示未接入，不把合成过敏树当运营目录。这不是 Query 里那扇独立 SOP 窗。

---

## 5. 停下来时记住的三句话

1. **synthetic-only**：未登录查询用仓内合成 fixture。产品远端下话术库读当前发布 hydrate；工作台缺合同的数字写「未接入」。
2. **no backend**：本页默认合成离线路径没有 PostgreSQL、飞书、对象存储、线上 API。工作台没有产品会话时写「未接入」。产品远端（`product-remote` / 隧道）另有 API，见 [P4 how-to](how-to-p4-remote-mac.md)。
...
下一步若要把仓外 FAQ 发成当前发布，打开 [管理员导入 MENOKIN FAQ 并发布](tutorial-menokin-content-publish.md)。核对测试与打包门禁，打开 [如何验证桌面 Demo](how-to-verify-desktop.md)。
