...
| 左侧能切到「管理概览」「VOC / 工单洞察」「SOP」等十个一期模块 | Publish 按钮可点；「工单垃圾桶」可进入 |

若打开失败，查询窗必须仍可用，并显示「工作台未打开，请重试。查询窗口仍保持可用。」不要把失败理解成「已经切到工作台」。

侧栏「演示环境」里可以再次看到完整边界：`MOCK AUTH / SYNTHETIC DATA / NO BACKEND`。

「内容与发布」可选择本地 CSV 或 xlsx，解析成待审核草稿。xlsx 只读第一张表。中文表头（快捷短语 / 产品话术 / 业务填写问题 / 客满话术）会映射到场景与标准话术，空白或不完整行会跳过。选文件会先显示「正在读取」。上限与后端对齐：10MiB / 5000 行。点 Publish 后会等导入完成再发布：状态大约每 1.5 秒问一次，遇到太频繁（429）会再等一轮，单次点击不会把接口打满。一期发布仅管理员（Owner）；话术师可导入产品或活动草稿。售后过敏树说明是 DEMO 样例，不接本页上传。

「话术优化待办」在有产品会话且角色为 coach / owner 时，走冻结 `GET /v1/metrics/iteration-tasks` 与 `POST .../start|close`。空列表合法，显示「当前没有待办」，不要当成故障。坐席会看到「坐席不能查看或处理话术优化待办」。没有产品会话时显示「当前没有产品会话，无法加载待办」，不会回落到 DEMO 5 条。冲突文案是「待办已更新，请刷新后再处理」。关闭待办不等于已发布。从 Query「话术不准」自动开单仍要 persist。

「SOP」在话术运营里，夹在话术优化待办和内容与发布之间。打开后是与坐席过敏窗同一份合成树的只读列表，kicker 为「合成过敏树只读 · 不编辑、不发布」，没有按钮、没有发布。coach / owner 登录后可见内部停手文案；坐席和会话失败时看不到。持久化仍要 contracts:intake。这不是 Query 里那扇独立 SOP 窗。
...
2. **no backend**：本页默认合成离线路径没有 PostgreSQL、飞书、对象存储、线上 API。Dashboard「无后端 · 不保存」只约束工作台演示。产品远端（`product-remote` / 隧道）另有 API，见 [P4 how-to](how-to-p4-remote-mac.md)。
