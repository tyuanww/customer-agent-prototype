...
「内容与发布」可选择本地 CSV 或 xlsx，解析成待审核草稿；中文表头（快捷短语 / 产品话术 / 业务填写问题 / 客满话术）会映射到场景与标准话术，空白或不完整行会跳过。选文件会先显示「正在读取」。上限与后端对齐：10MiB / 5000 行。一期发布仅管理员（Owner）；话术师可导入产品或活动草稿。售后过敏树说明是 DEMO 样例，不接本页上传。
