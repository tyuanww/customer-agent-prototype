...
- 索引、原文快照和正文向量在仓外，git 里没有 MENOKIN 正文。产品模式按 API origin 哈希分文件（`retrieval-*.<origin>.json`）；未设 origin 时才用未带后缀 leftover。每次产品查询都会 `refreshAnnounce`，新公告不必等 600s 租约。hydrate 文件仍在点胶囊「登录」时按当前公告回写（更小的种子 snapshot 不覆盖更大已有文件）；`STALE` 仍要点登录，不要 `stack start`。向量仍要 `pnpm retrieval:embeddings` 重算。
