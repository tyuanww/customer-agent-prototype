...
- Fox / Query renderer 只能通过类型化、白名单 preload API 请求原生能力，并按能力施加 sender / role / 必要时 main-frame 门禁。Dashboard 仅允许独立 `dashboard.ts` preload：话术库只读（`dashboard:wording-list`）、内容会话/解析/导入/发布（`dashboard:content-*`，含 `dashboard:content-parse`）、话术优化待办 list/start/close（`dashboard:iteration-*`）。不得挂 overlay 的 search / login / copy。未来正式只读 adapter 必须单独评审，禁止通用 `send/on/invoke`、任意 channel、任意窗口控制和文件系统能力。产品查询在存在 `productAnnounce` 时，每次点查询都先 `refreshAnnounce` 再检索，不只在第一次查询或登录时对齐公告。Dashboard 话术库优先当前发布 hydrate，hydrate 为空才回退本机检索索引。没有产品会话时工作台用空态 / 「未接入」，不得发明 fixture 数字。安装包自动更新未做。
